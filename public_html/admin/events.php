...
if(isset($_POST['addevent'])){
    ...
    if($_FILES['image']['name']) {
        ...
        if(!isset($_FILES['image']['name'])){
            $getimg = query("SELECT * FROM events WHERE id = '$id'");
    ...
    $sql = query("INSERT INTO events (title, date, time, location, gps, difficulty, duration, description, image) VALUES ('$title', '$date', '$time', '$loc', '$gps', '$difficulty', '$duration', '$description', '$image')");
    confirm($sql);
...
if(isset($_POST['editevent'])){
    ...
    $sql = query("UPDATE events SET title = '$title', date = '$date', time = '$time', location = '$loc', gps = '$gps', difficulty = '$difficulty', duration = '$duration', image = '$image', description = '$description' WHERE id = '$id'");
    confirm($sql);
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" class="mb-5">
        <h1 style="text-align: center;" class="mt-5 mb-5">Events</h1>
